...
import { getUserAssignedGroupsKey, getUserUnassignedGroupsKey } from "../AdminConstants"
import { useQuery } from "react-query"
import useAddRemoveTable from "../../../utils/useAddRemoveTable"

interface IAdminUserGroup {
  uId: number
}
...
const AdminUserGroup: React.FC<IAdminUserGroup> = ({ uId }: IAdminUserGroup) => {
  const form = useForm()
  const unassignForm = useForm()
  const assignForm = useForm()

  const { data: users } = useQuery(["admin-user-group", 0], () => getAdminUserList(0))

  const {
    data: assignedGroups,
    refetch: refetchAssigned,
    isFetching: isFetchingAssigned,
  } = useQuery(getUserAssignedGroupsKey(uId), () => getAdminUserAssignedGroupList(uId))
  const {
    data: unassignedGroups,
    refetch: refetchUnassigned,
    isFetching: isFetchingUnassigned,
  } = useQuery(getUserUnassignedGroupsKey(uId), () => getAdminUserUnassignedGroupList(uId))

  const loading = !users && uId > 0
  const { setLoading } = useLoader()
  const { toAdd, toRemove, clear, handleUserAdd, handleUserRemove } = useAddRemoveTable<AdminGroupModel>(
    assignedGroups ?? [],
    unassignedGroups ?? [],
    (item) => item.id
  )
  ...
  const onSubmit = async (data: any) => {
    ...
    Promise.allSettled([
      ...
      ...toRemove.map((e: AdminGroupModel) => deleteAdminGroupUser(e.id, uId).client()),
    ])
      .then((res) => {
        if (res.some((resItem: any) => resItem.value.status !== 200)) {
          throw new Error("Saved failed")
        }
        toast.success("Updated user groups.")
      })
      .catch((e) => {
        showToast(e, "Failed to update user groups.")
      })
      .finally(() => {
        setLoading(false)
      })
  ...
  useEffect(() => {
    if (selectedUser) {
      history.push(`/admin/${selectedUserId}/user-groups`)
  ...
  // console.log({ unassignForm: unassignForm.getValues() })
  // console.log({ assignForm: assignForm.getValues() })

  return (
    <div className="mt-4">
      <div className="d-flex ml-3 pl-3 mb-4">
        <h3 className="thruTitle ml-2">User Groups</h3>
        <div className="ml-2 mr-2 d-flex align-items-center thruSubtitle-breadcrumb-separator" />
        <div className="thruSubtitle d-flex align-items-center">manage user group assignment</div>
      </div>
      <div className="row ml-3 mr-4">
        ...
        <div className="col">
          <div className="thruCardContainer">
            <h4 className="ml-3 my-3">Assign User Groups</h4>
            <div className="p-5 border-top">
              <div className="row">
                <div className="col ml-3 px-0 manageUGP-Container-L">
                  ...
                  <div className="p-4">
                    <SearchableTable
                      ...
                      data={[
                        ...(unassignedGroups?.filter((item) => !toAdd.some((curr) => curr.id === item.id)) ?? []),
                        ...toRemove,
                      ]}
                      ...
                      isFetching={isFetchingUnassigned}
                      onFormSubmit={(data) =>
                        handleUserAdd(data, unassignForm, "checkbox-main-assign", "checkbox-assign-")
                      }
                ...
                >
                  ...
                  >
                    <h4 className="pb-3 pl-3 pt-3">Assigned User Groups</h4>
                  </div>
                  <div className="p-4">
                    <SearchableTable
                      header={header}
                      ...
                      isFetching={isFetchingAssigned}
                      data={[
                        ...(assignedGroups?.filter((item) => !toRemove.some((curr) => curr.id === item.id)) ?? []),
                        ...toAdd,
                      ]}
                      onFormSubmit={(data) =>
                        handleUserRemove(data, assignForm, "checkbox-main-unassign", "checkbox-unassign-")
                      }
              ...
              <form onSubmit={form.handleSubmit(onSubmit)}>
                <div className="row">
                  <div className="col d-flex">
                    <div className="mr-3">
                      <button id="btnSaveUserGroups" className="btn btn-Save btn-thru-lg">
                        Save
                      </button>
                    </div>
                    <div className="mr-3">
